...
```toml
[dependencies]
ncp-core  = { git = "https://github.com/sepahead/NCP", tag = "v0.4.3" }
ncp-zenoh = { git = "https://github.com/sepahead/NCP", tag = "v0.4.3" }  # transport, optional
```
...
- [`CHANGELOG.md`](CHANGELOG.md) — per-release notes (current: `v0.4.3`).

## Status

NCP is **pre-1.0 and experimental.** Specifically:

- **The wire may change.** Minor versions are treated as breaking; the version guard fails closed rather than coercing. **Pin the latest tag** (`tag = "v0.4.3"` above — the wire is `0.4`, with `v0.4.3` the buf-breaking baseline) for anything you build against.
